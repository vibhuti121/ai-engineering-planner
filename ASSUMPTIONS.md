...
**15. The prompt version must be bumped by hand when the prompt changes.**
`PROMPT_VERSION` in `app/config.py` is part of the cache key. Hashing the prompt file itself would
make this automatic; a constant was chosen so that whitespace and comment edits do not needlessly
invalidate every cached plan. The trade is that forgetting to bump it serves stale plans — noted at
...
**19. Tests cover the ordering module, not the whole app.**
...
each malformed-graph case. The HTTP layer, renderer, and validator were verified by running them
end to end and are documented in the README's verification commands. Given more time, the validator
and the parser's fallback paths are the next two worth unit tests.

**20. The sample PRD is synthetic and deliberately unrelated to any real product.**
